...
2.x ベータ系：**v2.0.0-beta.3**。複数位置への表示とアカウント全体の利用ペース予測を開発する系統で、ベータ受け入れ確認が終わるまでは Latest の安定版を置き換えません。

| プラットフォーム | 現在の状態 |
|---|---|
| Windows 11 x64 | ✅ 安定 / サインイン済み実機で確認済み |
| Windows 10 x64（2004 以降） | ✅ 対応基準 · 実機からの報告を歓迎 |
| Windows 11 ARM64 | 🧪 x64 エミュレーション · CI 検証待ち |
| Windows 10 ARM64 | ❌ 未対応 · x64 版 Codex をエミュレーションできません |
...
- 残りパーセント
- 公式の残りパーセント推移から求める直近の消費ペースと短い使用可能見込み（予測幅はホバーに表示）
...
2.x の複数位置ベータでは、長押し領域も実際の表示位置へ移動します。タイトル中央、ワークスペース上部、入力欄ツールバー、アカウント欄で、同じ長押し・ドラッグ・衝突回避・整列ロジックを使います。操作範囲が不安定だった入力欄の左右スロットは削除しました。
