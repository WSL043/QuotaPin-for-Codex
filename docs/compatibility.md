...
- **2.0.0-beta.1:** introduces schema 18 semantic placement zones. The isolated production renderer verifies separate title-bar and workspace-top slots, account and workspace-side slots, composer-safe slots, a rail outside the painted composer shell rather than an inner editor wrapper, fail-closed fallback, native account identity, stable keyed remote modules, remote hold entry, identical framed reordering at all five remote zones, a panel anchored beside the invoked surface, stable panel ownership, and unchanged Codex account-row short-click routing. It is not a stable artifact and does not replace v1.2.0 as Latest.
